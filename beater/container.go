...
func (a *Ampbeat) updateEventsStream() {
...
func (a *Ampbeat) startEventStream(stream <-chan events.Message, errs <-chan error) {
...
func (a *Ampbeat) updateContainerMap(action string, containerID string) {
	if action == "start" {
...
func (a *Ampbeat) addContainer(ID string) {
...
func (a *Ampbeat) removeContainer(ID string) {
...
func (a *Ampbeat) updateContainer(ID string) {
	data, ok := a.containers[ID]
	if ok {
		inspect, err := a.dockerClient.ContainerInspect(context.Background(), ID)
		if err == nil {
...
func (a *Ampbeat) getMapValue(labelMap map[string]string, name string) string {
...
// Close Ampbeat ressources
func (a *Ampbeat) Close() {
	a.closeLogsStreams()
